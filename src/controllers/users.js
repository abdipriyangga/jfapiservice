...
const { BASE_URL_ONLINE } = process.env;
...
exports.createUser = async (req, res) => {
  ...
  try {
    ...
    const checkNumberPasspor = await authModel.getUserByPassporNumber([
      data.pasporNumber,
    ]);
...
exports.selectUserByDateDeparture = async (req, res) => {
  ...
  try {
    ...
    const data = new Date(+year, +month - 1, +day).toLocaleDateString();
...
exports.addMemo = async (req, res) => {
  ...
  try {
    await userModel.addMemo(
      [data.memoMessage, role],
      (err, results, _fields) => {
        if (!err) {
          if (!err) {
            Messaging.send({
...
exports.logout = async (req, res) => {
  try {
    if (req.headers.authorization) {
      await userModel.logoutUsers(
        ["false", req.authUser.id],
        (err, results, _fields) => {
          console.log(results);
          if (!err) {
...
exports.getDataDepartureFromApiMarkeplace = async (req, res) => {
  try {
    const callData = await fetch(
      `${BASE_URL_ONLINE}adm-ticketing/allDepartureDate`,
      {
        method: "GET",
        headers: {
...
exports.getDataHotelFromHotelsMaster = async (req, res) => {
  try {
    console.log(BASE_URL_ONLINE);
    const callData = await fetch(
      `${BASE_URL_ONLINE}adm-hotel/hotel-master/all-data-service`,
      {
        method: "GET",
        headers: {
          "Content-Type": "application/json",
